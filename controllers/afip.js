...
exports.addVoucher = async (req, res, next) => {
    ...
    const bi21 = req.body.bi21;
    const bi27 = req.body.bi27;
    const iva10 = req.body.iva10;
    const iva21 = req.body.iva21;
    const iva27 = req.body.iva27;
    const totalIva = req.body.totalIva;
    const totalNoIva = req.body.totalNoIva;
    if (bi10 !== 0 && bi21 !== 0 && bi27 !== 0) {
        data2 = {
            ...
            ImpIVA: totalIva,  //Importe total de IVA
    ...
    //     'CbteHasta': parseInt(req.body.CbteHasta) + 1,  // Número de comprobante o numero del último comprobante en caso de ser mas de uno
    //     'CbteFch': parseInt(date.replace(/-/g, '')), // (Opcional) Fecha del comprobante (yyyymmdd) o fecha actual si es nulo
    //     'ImpTotal': parseFloat((total).toFixed(2)), // Importe total del comprobante
    //     'ImpTotConc': (req.body.ImpTotConc),   // Importe neto no gravado
    //     'ImpNeto': parseFloat((total / 121 * 100).toFixed(2)), // Importe neto gravado
    //     'ImpOpEx': (req.body.ImpOpEx),   // Importe exento de IVA
    //     'ImpIVA': parseFloat((total / 121 * 21).toFixed(2)),  //Importe total de IVA
    //     'ImpTrib': (req.body.ImpTrib),   //Importe total de tributos
    //     'MonId': req.body.MonId, //Tipo de moneda usada en el comprobante (ver tipos disponibles)('PES' para pesos argentinos) 
    //     'MonCotiz': (req.body.MonCotiz), // Cotización de la moneda usada (1 para pesos argentinos) 
    //     'Iva': [ // (Opcional) Alícuotas asociadas al comprobante
    //         {
    //             'Id': 4, // Id del tipo de IVA (5 para 21%)(ver tipos disponibles) 
    //             'BaseImp': parseFloat((total / 121 * 100).toFixed(2)), // Base imponible
    //             'Importe': parseFloat((total / 121 * 21).toFixed(2)) // Importe 
    //         },
    //         {
    //             'Id': 5, // Id del tipo de IVA (5 para 21%)(ver tipos disponibles) 
    //             'BaseImp': parseFloat((total / 121 * 100).toFixed(2)), // Base imponible
